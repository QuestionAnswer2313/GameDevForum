...
{
    f32* matrixArray = (f32*)&tc->Transform.M[0];
    JsonValue jsonValue = JSON_F32_ARRAY_NEW(matrixArray, 16);
...
get_sprite_component(SpriteComponent* sc)
{
    ...
    JsonValue colorJsonValue = JSON_F32_ARRAY_NEW(sc->Color.V, 4);
...
#define GetJsonValues(compValue)					\
	...
	for (c = 0; c < compsCount; ++c)
	{
		...
		tc.Transform = m4_array((f32*)vals[0].Data);

...
_ecs_entity_get_archetype(World* world, EntityID entityID)
{
    ...
    const ComponentID* ids = _ecs_entity_get_components_id(world, entityID);
...
*/
typedef struct ComponentsNameToID
{
    const char* FullName;
    char** SplittedNames;
    ComponentID* ID;
...
static ComponentsNameToID* ComponentsNameToIDCache = NULL;
...
components_name_to_id(World* world, const char* components)
{
    ...
    ComponentsNameToID compCache = array_find_predicate(ComponentsNameToIDCache, string_compare(item.FullName, components));
    if (compCache.FullName != NULL)
    {
	return compCache.ID;
    }

    char** splittedNames = string_split((char*)components, ',');
    i32 i, count = array_count(splittedNames);
    for (i = 0; i < count; i++)
    {
	ComponentID id = shash_get(world->Storage.NameToID, splittedNames[i]);

	{ // Debug check, not sure we need this
	    i32 index = array_find_index(ids, id);
	    if (index != -1 && count > 0)
	    {
		GERROR("All components: %s \n", components);
		array_foreach(splittedNames, GERROR("Component: %s\n", item););
		vassert(0 && "Can't id is already in array;");
	    }
	}
    ...
    ComponentsNameToID cache = {
	.FullName = components,
	.SplittedNames = splittedNames,
	.ID = ids
    };

    array_push(ComponentsNameToIDCache, cache);

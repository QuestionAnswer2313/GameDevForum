...
scene_on_update(Scene* scene, const Camera* camera)
{
	...
	DO_MANY_TIME(GLOG("Color: %f %f %f\n", sprite->Color.R, sprite->Color.G, sprite->Color.B), 2);
	...
	//NOTE(bies): not using scene for now
...
scene_entity_create(Scene* scene, const char* name)
{
    ...
    ECS_ENTITY_SET_COMPONENT(scene->World, entityId, TransformComponent, TransformComponent_(v3_new(1.0f, 1.0f, 1.0f), v3_new(1.0f, 1.0f, 0.0f), v3_new(0.0f, 0.0f, 0.0f)));
...
scene_entity_destroy(Scene* scene, Entity entity)
{
    ...
    array_remove_at(scene->Entities, i);
